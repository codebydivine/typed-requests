...
class TestTypeValidationPatterns:
    ...
    async def test_validation_error_handling(self):
        ...
        with pytest.raises(ValidationError):
            await manager.get(
                "https://api.example.com/strict",
                expected_type=StrictResponse
            )
    ...
    async def test_authentication_pattern(self):
        ...
        class AuthenticatedClient:
            ...
            async def get_protected_resource(self, resource_id: int):
                ...
                response = await self.manager.get(
                    f"https://api.example.com/protected/{resource_id}",
...
class TestLifecycleManagement:
    ...
    async def test_startup_shutdown_pattern(self):
        ...
        original_client = manager._client
        mock_client = AsyncMock(spec=httpx.AsyncClient)
        mock_client.request.return_value = mock_response
        manager._client = mock_client
    ...
    async def test_example_imports(self):
        ...
        try:
            ...
            spec = importlib.util.spec_from_file_location(
                ...
                os.path.join(examples_dir, "api_response_validation.py")
            )
            ...
            spec = importlib.util.spec_from_file_location(
                "advanced_api_integration", 
                os.path.join(examples_dir, "advanced_api_integration.py")
            )
            ...
            spec = importlib.util.spec_from_file_location(
                "e_commerce_api_client", 
                os.path.join(examples_dir, "e_commerce_api_client.py")
            )
...
class TestExampleIntegration:
    ...
    async def test_full_workflow_integration(self):
        ...
        user_detail_response.json.return_value = {
            "id": 1,
            "name": "User 1",
        ...
        # Test workflow
        # 1. Get list of users
        users_response = await manager.get(
            "https://api.example.com/users",
            expected_type=UserListResponse
        )
        ...
        user_response = await manager.get(
